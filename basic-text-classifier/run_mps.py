...
training_args = TrainingArguments(
    ...
    save_strategy="epoch",
    learning_rate=2e-5,
    per_device_train_batch_size=32,  # Increased batch size for M4 Pro
    per_device_eval_batch_size=32,   # Increased batch size for M4 Pro
    num_train_epochs=1,
    weight_decay=0.01,
    load_best_model_at_end=True,
    metric_for_best_model="f1",
    ...
    logging_steps=100,
    report_to="none",
    # fp16=True,  # Enable mixed-precision training for performance boost
    use_mps_device=True # Explicitly tell the Trainer to use the MPS device
)
...
def classify_prompt(prompt):
    inputs = tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=128).to(device) # Move inputs to the MPS device
    model.eval()
    with torch.no_grad():
        outputs = model(**inputs)
        logits = outputs.logits
